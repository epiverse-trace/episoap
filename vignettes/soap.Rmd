...
The reports are classic `{rmarkdown}` file.
...
```{r}
# Load template in temporary directory for this vignette. You will probably wish to save it somewhere else.
rmarkdown::draft(
...
webshot::rmdshot(
  system.file("rmarkdown", "templates", "transmissibility-report", "skeleton", "skeleton.Rmd", package = "soap"),
  file = "vignettes/transmissibility-EpiEstim.png",
  rmd_args = list(
    params = list(rt_estimator = "EpiEstim"),
    quiet = TRUE
  )
...
webshot::rmdshot(
  ...
  file = "vignettes/transmissibility-EpiNow2.png",
  rmd_args = list(
    params = list(rt_estimator = "EpiNow2"),
    quiet = TRUE
  )
