...
ggplot(res_epiestim_global, aes(x = end)) +
  ...
  theme_custom +
  labs(x = "", y = "Instantaneous reproduction number (Rt)")
...
dat_i_day_recent <- dat_i_day %>%
  keep_last(14)
res_epiestim_group <- dat_i_day_recent %>% 
  nest(data = c(date_index, n)) %>%
  mutate(res_epiestim = map(data, ~ wrap_res(estimate_R(.$n, config = ee_config), dat_i_day_recent))) %>%
  unnest(res_epiestim) %>%
  select(-data) %>% 
  filter(end == max(end))
...
res_epiestim_group %>%
  ...
  labs(title = "Estimates of Rt (EpiEstim)",
       subtitle = sprintf(
         "based on data from %s - %s",
         format(max(get_dates(dat_i_day_recent) - 7), "%d %B %Y"),
         format(max(get_dates(dat_i_day_recent)), "%d %B %Y")),
       y = "",
...
#  select(date, everything()) %>%
  ...
  kbl() %>%
  kable_paper("striped", font_size = 18, full_width = FALSE) %>%
  row_spec((1:nrow(res_epiestim_group))[c(FALSE, TRUE)],
           background = pale_green)
