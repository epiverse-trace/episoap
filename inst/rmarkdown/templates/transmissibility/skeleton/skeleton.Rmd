...
To illustrate the different analyses, we use data reporting daily numbers of 
[enter disease name] in [enter location] during [enter timeframe], 
and is stratified by [enter region level]. 
The data is available from [enter data source].

The data file is named "*file_name.rds*" and is located in
the *data/* folder.
...
* `date`: the date of [enter description]
* `group`: [enter description of grouping variable, e.g., region, age, gender]
...
This section creates epidemic curves ("_epicurves_"), with and without stratification by [name of chosen grouping variable].
...
This graph shows the total number of cases per region:
...
In this report, the mean and standard deviation of the $si$ have been [provided as an input by the author/
obtained from a library of epidemiological parameters from the {epiparameter} R package]
