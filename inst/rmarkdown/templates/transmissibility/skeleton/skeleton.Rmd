...
Once imported into __R__, the dataset used in this report is named `dat_raw`, and contains the following variables:
...
```
```{r, include=FALSE}
min_date <- min(dat_raw$date)
max_date <- max(dat_raw$date)
```
