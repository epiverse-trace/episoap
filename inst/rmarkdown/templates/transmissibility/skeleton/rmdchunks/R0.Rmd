...
# Plot of latest estimates of Rt, defined by params$incompolete_days
res_r0_group %>%
  na.omit() %>%
...
```{r rt-table-group}
# This code generates a table with latest estimates of Rt, defined by
# params$incomplete_days
res_r0_group %>%
  na.omit() %>%
  filter(date_index == max(date_index)) %>%
  mutate(
