...
```{r process-dates-onset-death, eval = params$data_columns != "Dates of onset and total deaths only", echo=params$data_columns != "Dates of onset and total deaths only"}
...
```{r process-dates-onset-only, eval = params$data_columns == "Dates of onset and total deaths only", echo=params$data_columns == "Dates of onset and total deaths only"}
# This code converts daily incidence into weekly incidence using {incidence2} for cases only
dat_i <- dat_raw %>%
    incidence(date_index = cases,
    interval = 1L,  
    counts = count_var,
    groups = group_var
  )
...
o_d_mean <- params$onset_death_mean
o_d_sd <- params$onset_death_sd
o_d_dist <- params$onset_death_dist
